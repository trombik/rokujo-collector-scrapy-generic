...
class ReadMoreSpider(GenericSpider[ReadMoreSpiderConfig], ReadMoreMixin):
    """
    A spider to extract an article from a landing page, the main article page,
    and "Next" pages.  It also supports a single page of an article. This
    spider is useful when RSS feed does not return the link to the main
    article but a landing page.

    This spider processes summary pages that contain links to main articles.
    For example, a summary page might have a link like
    ...
    * Main article page
    ...
    :class:`generic.items.ArticleItem` when the page contains a read_more
    link. Otherwise, the content is included as part of the article.

    When the main article is split into multiple pages, specify ``read_next``.
    The spider crawls all the pages and returns a single ArticleItem.

    The spider accepts a comma-separated list of summary page URLs and returns
    ArticleItem of the main articles.

    When no link with read_more text is found, the spider parses the summary
    ...
    Args:
        ...
        read_more:
            Text string of the ``<a>`` tag that links to the main article.
            Default is ``記事全文を読む``.
        ...
        read_more_xpath:
            ...
            1. ``//h3``
                Look everywhere: Search the entire document for any Level 3
                Heading (``<h3>``).
